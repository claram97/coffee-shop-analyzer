...
class Orchestrator:
    ...
    def __init__(self, port: int, listen_backlog: int):
        ...
        fr_q = MessageMiddlewareQueue(rabbitmq_host, "filter.router.in")

        self._filter_router_queue = fr_q
        self.message_handler = MessageHandler(fr_q)
    ...
    def _process_query_request(self, msg, client_sock) -> bool:
        ...
        try:
            ...
            logging.error(
                f"action: query_request_processing | result: fail | error: {e}"
            )
            return ResponseHandler.handle_processing_error(msg, client_sock, e)
