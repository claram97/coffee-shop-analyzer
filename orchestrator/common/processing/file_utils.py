...
class MessageLogger:
    ...
    def __init__(
        self,
        original_file: str = "received_messages.txt",
        filtered_file: str = "filtered_messages.txt",
    ):
        """
    ...
    def write_original_message(self, msg: Any, status_text: str):
        ...
        try:
            with open(self.original_file, "a", encoding="utf-8") as f:
                f.write(
                    f"=== Original Message - Opcode: {msg.opcode} - Amount: {msg.amount} - Batch: {msg.batch_number} - Status: {status_text} ===\n"
                )
            ...
            logging.warning(
                f"action: write_original_message | result: fail | error: {e}"
            )
    ...
    def write_filtered_message(self, filtered_batch: Any, status_text: str):
        ...
        try:
            with open(self.filtered_file, "a", encoding="utf-8") as f:
                f.write(
                    f"=== Filtered Message - Table: {filtered_batch.filtered_data['table_name']} - Original: {filtered_batch.filtered_data['original_row_count']} - Filtered: {filtered_batch.filtered_data['filtered_row_count']} - Batch: {filtered_batch.filtered_data['batch_number']} - Status: {status_text} ===\n"
                )
                f.write(f"Query IDs: {filtered_batch.query_ids}\n")
                f.write(f"Table IDs: {filtered_batch.table_ids}\n")
                f.write(f"Shards Info: {filtered_batch.shards_info}\n")
                for i, row in enumerate(filtered_batch.filtered_data["rows"]):
                    f.write(f"Row {i+1}: {row}\n")
                f.write("\n")
        except Exception as e:
            logging.warning(
                f"action: write_filtered_message | result: fail | error: {e}"
            )
