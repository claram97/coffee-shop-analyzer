...
: "${HEARTBEAT_MAX_MISSES:=6}"
: "${HEARTBEAT_STARTUP_GRACE_SECONDS:=6.0}"
: "${HEARTBEAT_ELECTION_COOLDOWN_SECONDS:=10.0}"
: "${HEARTBEAT_COOLDOWN_JITTER_SECONDS:=0.5}"
: "${FOLLOWER_DOWN_TIMEOUT_SECONDS:=15.0}"
: "${FOLLOWER_RESTART_COOLDOWN_SECONDS:=45.0}"
: "${FOLLOWER_RECOVERY_GRACE_SECONDS:=10.0}"
: "${FOLLOWER_MAX_RESTART_ATTEMPTS:=100}"
: "${ORCH_PROCESS_QUEUE_RETRY_SLEEP:=0.3}"
...
services:
  ...
  orchestrator:
    ...
    environment:
      ...
      - ORCH_PROCESS_QUEUE_SIZE=1024
      - ORCH_PROCESS_QUEUE_TIMEOUT=300.0
      - ORCH_PROCESS_QUEUE_RETRY_SLEEP=${ORCH_PROCESS_QUEUE_RETRY_SLEEP}
    networks:
      - testing_net
    volumes:
